...
class BSSVAELoss(LossParent):
    ...
    def slice_x(self, x, which):
        len_dim = self.max_len_tcr if which == 'tcr' else self.max_len_pep
        matrix_dim = self.aa_dim
        if self.add_positional_encoding:
            matrix_dim += self.pos_dim_tcr

        sequence = x[:, 0:(len_dim * matrix_dim)].view(-1, len_dim, matrix_dim)
        positional_encoding = None
        if self.add_positional_encoding:
            positional_encoding = sequence[:, :, self.aa_dim:]
            sequence = sequence[:, :, :self.aa_dim]
    ...
    def _kld_weight_regime(self):
        ...
        # TanH warm-up phase
        if self.counter <= self.kld_warm_up:
            self.weight_kld_n = self._tanh_annealing()
        ...
        # Start to decrease weight once counter > warm_up+flat phase
