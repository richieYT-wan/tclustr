...
def trim_graph_into_subgraphs(G, size_cutoff, priority_nodes=set(),
    ...
    while flag == False:
        ...
        if priority_nodes:
            ...
            print(diff)
            if diff:
                nodes_to_add = [central_sort_nodes[n] for n in range(i) if central_sort_nodes[n] not in bottleneck_nodes]
        ...
        #we are done
        if flag: print("We done!")
        
        #we can simplify the graph, before continueing 
        elif anyflag:
            keep_subgraphs = []
            for i in range(N):
                if check[i]: trimmed_subgraphs.append(subgraphs[i])
                else: keep_subgraphs.append(subgraphs[i])
