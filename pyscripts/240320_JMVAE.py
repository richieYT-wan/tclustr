...
from src.utils import str2bool, pkl_dump, mkdirs, plot_vae_loss_accs, \
...
def args_parser():
    ...
    parser.add_argument('-b3', '--b3_col', dest='b3_col', default='B3', type=str, required=False,
                        help='Name of the column containing B3 sequences (inputs)')
    # These two pair args exist for compatibility reasons ; Should always be true
    parser.add_argument('-pair_only', dest='pair_only', default=True, type=str2bool, required=False,
                        help='Whether to "trick" and use the paired_only data')
    parser.add_argument('-return_pair', dest='return_pair', default=True, type=str2bool, required=False,
                        help='Return pairs only from the dataloader')
...
def main():
    ...
    model_keys = get_class_initcode_keys(JMVAE, args)
    dataset_keys = get_class_initcode_keys(MultimodalPepTCRDataset, args)
    loss_keys = get_class_initcode_keys(JMVAELoss, args)
    ...
    model = JMVAE(**model_params)
    model.to(device)
    criterion = JMVAELoss(**loss_params)
    criterion.to(device)
