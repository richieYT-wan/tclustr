...
def main():
    ...
    latent_df_os_notrp = get_latent_df(model_os_notrp, df)
    latent_df_ts_notrp = get_latent_df(model_ts_notrp, df)
    latent_df_os_cstrp = get_latent_df(model_os_cstrp, df)
    latent_df_ts_cstrp = get_latent_df(model_ts_cstrp, df)

    if index_col is None or index_col=='' or index_col not in latent_df_os_cstrp.columns:
        index_col = 'index_col'
        latent_df_os_cstrp[index_col] = [f'seq_{i:04}' for i in range(len(latent_df_os_cstrp))]
    seq_cols = tuple(args[f'{x.lower()}_col'] for x in ('A1', 'A2', 'A3', 'B1', 'B2', 'B3'))

    dm_vae_os_notrp, vals_vae, _, labels, encoded_labels, label_encoder = get_distances_labels_from_latent(latent_df_os_notrp,
                                                                                                  label_col, seq_cols,
                                                                                                  index_col,
                                                                                                  rest_cols,
                                                                                                  args['low_memory'])
    dm_vae_ts_notrp, vals_vae, _, labels, encoded_labels, label_encoder = get_distances_labels_from_latent(latent_df_ts_notrp,
                                                                                                  label_col, seq_cols,
                                                                                                  index_col,
                                                                                                  rest_cols,
                                                                                                  args['low_memory'])
    dm_vae_os_cstrp, vals_vae, _, labels, encoded_labels, label_encoder = get_distances_labels_from_latent(latent_df_os_cstrp,
                                                                                                  label_col, seq_cols,
                                                                                                  index_col,
                                                                                                  rest_cols,
                                                                                                  args['low_memory'])
    dm_vae_ts_cstrp, vals_vae, _, labels, encoded_labels, label_encoder = get_distances_labels_from_latent(latent_df_ts_cstrp,
                                                                                                  label_col, seq_cols,
                                                                                                  index_col,
                                                                                                  rest_cols,
                                                                                                  args['low_memory'])
