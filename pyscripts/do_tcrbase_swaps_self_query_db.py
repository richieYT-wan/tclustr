...
from src.sim_utils import make_self_db_distmatrix
from src.cluster_utils import get_latent_df
...
"""
Script to run the TCRbase method with a VAE latent space but using "itself" as query and database 
Takes only the positives (self) as database, and will ignore self score 
"""
...
def do_tcrbase_and_histplots(preds, peptide, f=None, ax=None,
                             unique_filename=None, outdir=None, bins=100):

    dist_matrix, db, query = make_self_db_distmatrix(preds, peptide)
    ...
    pos_out = pos.apply(lambda x: [np.min(x), x.index[int(np.argmin(x))]], axis=1, result_type='expand').rename(
        columns={0: 'min_dist', 1: 'hit'}).assign(label=1)
    neg_out = neg.apply(lambda x: [np.min(x), x.index[int(np.argmin(x))]], axis=1, result_type='expand').rename(
        columns={0: 'min_dist', 1: 'hit'}).assign(label=0)
    cat_out = pd.concat([pos_out, neg_out])
    auc = roc_auc_score(cat_out['label'], 1 - cat_out['min_dist'])
    cat_out = pd.merge(cat_out, dist_matrix[['original_peptide']], left_index=True, right_index=True).sort_values(
    ...
    pos_flat = pos[~np.isnan(pos)].flatten()
    neg_flat = neg[~np.isnan(neg)].flatten()
    ...
    if unique_filename is not None:
        ...
        f.savefig(f'{outdir}{peptide}_AvA_SelfDBTCRBase_distances_histplot_{unique_filename}', dpi=150,
                  bbox_inches='tight')
    ...
    if unique_filename is not None:
        outdir = './' if outdir is None else outdir
        f2.savefig(f'{outdir}{peptide}_Best_SelfDBTCRBase_distances_histplot_{unique_filename}', dpi=150, bbox_inches='tight')

    return cat_out


def wrapper(preds, peptide, unique_filename, outdir):
    cat_out = do_tcrbase_and_histplots(preds, peptide,
...
def args_parser():
    ...
    parser.add_argument('-f', '--file', dest='file', required=True, type=str,
                        default='../data/filtered/240418_nettcr_expanded_20binders_17pep_POSONLY.csv',
                        help='filename of the input file. Will be used as query AND database')
...
def main():
    ...
    if isinstance(vae, JMVAE) or isinstance(vae, BSSVAE):
        ...
        dataset = MultimodalPepTCRDataset(df, **dataset_params)
        loader = dataset.get_dataloader(batch_size=2048, sampler=SequentialSampler)
        preds = embed_multimodal(vae, dataset, loader)
        ...
        preds = predict_model(vae, dataset, loader)

    # Here, concatenate and construct the distance matrix a single time ; Keep the columns for filtering later
    preds['tcr'] = preds.apply(lambda x: ''.join([x[c] for c in ['A1', 'A2', 'A3', 'B1', 'B2', 'B3']]), axis=1)

    with open(f'{outdir}args_{unique_filename}.txt', 'w') as file:
        for key, value in args.items():
            file.write(f"{key}: {value}\n")

    wrapper_ = partial(wrapper, preds=preds, unique_filename=unique_filename, outdir=outdir)
    # Then, on a per-peptide basis, do the TCRbase method
    text = Parallel(n_jobs=6)(
        delayed(wrapper_)(peptide=peptide) for peptide in tqdm(preds.peptide.unique(), desc='peptide'))
