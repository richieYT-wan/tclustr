...
def bruteforce(n_clusters, ODIR, z_train_30k, z_valid_30k, z_gil_30k, train_preds_30k, valid_preds_30k, gil_preds_30k):
    for kernel in tqdm(['rbf', 'laplacian', 'cosine'], desc='sc kernel', position=1, leave=False):
        sc = SpectralClustering(n_clusters, affinity=kernel, n_init=30, random_state=13, n_jobs=9)
...
Parallel(n_jobs=4)(delayed(ntm)(n_clusters=n_clusters) for n_clusters in tqdm(range(2, 50), desc='n_clst', position=0, leave=True))
end = dt.now()
