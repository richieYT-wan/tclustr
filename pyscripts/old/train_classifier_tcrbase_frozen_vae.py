...
def do_tcrbase(dist_matrix, ref, label='GILGFVFTL'):
    output = dist_matrix.drop(columns=[x for x in dist_matrix.columns if x != 'label']).copy().reset_index().rename(
        columns={'index': 'tcr'})
...
def tcrbase_pipeline(preds, dist_type, ref, peptide='GILGFVFTL'):
    ...
    dist_matrix['set'] = ref['set'].values
    dist_matrix['label'] = ref['peptide'].values
    dist_matrix['binder'] = ref['binder'].values
    dist_matrix['origin'] = ref['origin'].values
    train = dist_matrix.query('set=="train" and label==@peptide and binder==1')
    valid = dist_matrix.query('set=="valid" and binder==1')
    # print(ref.query('set=="valid"').groupby('partition').agg(count=('B3','count')))
    results = do_tcrbase(valid, train, label=peptide)
    if 'cos' in dist_type:
        results['score'] = 1 - results['best_sim']
    else:
        results['score'] = 100 - results['best_sim']
    return results
