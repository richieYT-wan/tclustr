...
def fix_json(directory):
    ...
    try:
        ...
        # Here, if stopped iteration that means there was not even a json file, so no need to delete the old
        kwargs = get_kwargs(files)
        js_file_new = pt_file.replace('.pt', '.json').replace('checkpoint','checkpoint_FIXED_KWARGS')
        try:
            save_json(kwargs, js_file_new)
            model=load_model_full(pt_file, js_file_new)
        except: 
            os.remove(js_file_new)
            raise ValueError('Couldn\'t load model with the "fixed" kwargs!')

    except json.decoder.JSONDecodeError:
        # Here, it means it tried to load the JSON and it exists but failed to decode (due to the serialization error)
        kwargs = get_kwargs(files)
        js_file_new = pt_file.replace('.pt', '.json').replace('checkpoint','checkpoint_FIXED_KWARGS')
        try:
            save_json(kwargs, js_file_new)
            model=load_model_full(pt_file, js_file_new)
            # here deletes the old file now
            os.remove(js_file)
        except:
            os.remove(js_file_new)
            raise ValueError('Couldn\'t load model with the "fixed" kwargs!')
