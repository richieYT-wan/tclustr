  ...
  for kf in $(seq 0 4); do
    ...
    for folder in $f; do
      if [ -d "$folder" ]; then
        ...
        python3 ./train_classifier_frozen_vae.py -cuda True -f ../data/filtered/231205_nettcr_old_26pep_with_swaps.csv -o "Flip_BLOSUM/CLF_1layer64_025_BN_withSwaps_FLIP_BLOSUM_${outname}" -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 1e-4 -bs 1024 -ne 1000 -pepenc BL50LO -kf ${kf} -rid "${id}" -seed ${kf} -model_folder "../output/VAE_For_CLF/${mainfolder}/${folder}/"
      fi
    done
  done
