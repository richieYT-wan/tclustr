    ...
    if [[ $(ls ../output/${outdir} | grep ${random_id} | wc -l) -ne 1 ]]; then
      ...
      for kf in $(seq 0 4);do
        ...
        for folder in ${kfolder};do
          if [ -d ${kfolder} ]; then
            python3 ./train_classifier_frozen_vae.py -model_folder "${folder}/" -rid ${random_id} -od ${outdir}/${folder_name}/ -o "CLF_${name_description}" -cuda True -f ../data/filtered/231205_nettcr_old_26pep_with_swaps.csv -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 1e-5 -bs 1024 -ne ${1} -pepenc BL50LO -kf ${kf} -seed ${kf} -pepweight True
          fi
        done
      done
    else
