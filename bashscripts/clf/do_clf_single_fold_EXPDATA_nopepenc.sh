...
mainfolder='../output/vae_models'
outdir='../output/vae_clfs_output/'
grepstatement=$4
...
for fullpath in ${subdir}; do
    ...
    python3 ./train_classifier_frozen_vae.py -json_file ${json_file} -pt_file ${best_checkpoint} -rid ${random_id} -od "${outdir}/BEST_${folder_name}/" -o "BEST_${name_description}" -cuda True -f ../data/multimodal/240326_nettcr_paired_withswaps.csv -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 5e-6 -bs 2048 -ne 1250 -pepenc BL50LO -kf 0 -seed 0 -pepenc 'none' -pepweight False
    python3 ./train_classifier_frozen_vae.py -json_file ${json_file} -pt_file ${last_checkpoint} -rid ${random_id} -od "${outdir}/LAST_${folder_name}/" -o "LAST_${name_description}" -cuda True -f ../data/multimodal/240326_nettcr_paired_withswaps.csv -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 5e-6 -bs 2048 -ne 1250 -pepenc BL50LO -kf 0 -seed 0 -pepenc 'none' -pepweight False
    echo "####################"
done
