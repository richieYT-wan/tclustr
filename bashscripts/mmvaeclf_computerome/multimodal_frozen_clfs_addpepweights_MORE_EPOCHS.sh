...
outdir='/home/projects/vaccine/people/yatwan/tclustr/output/mmvae_clfs_output_PEPWEIGHTED/'
mkdir -p ${outdir}
n_epochs=3000
grep_statement=VAE
grep_v_statement=xd
...
logfile=/home/projects/vaccine/people/yatwan/tclustr/logs/NTM_multimodal_frozen_clfs_addpepweights_MORE_EPOCHS.log
touch $logfile
...
#    echo "#######################################"
    ...
    python3 ./train_classifier_frozen_mmvae.py -json_file ${json_file} -pt_file ${best_checkpoint} -rid ${random_id} -od ${outdir} -o "BEST_3kEpochs_CC_${name_description}" -cuda True -f ../data/multimodal/240326_nettcr_paired_withswaps.csv -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 5e-6 -bs 2048 -ne ${n_epochs} -pepenc 'none' -kf 0 -seed 0 -pepweight True -device cuda:0 >> $logfile 2>&1
    # Doing with "last" checkpoint
    python3 ./train_classifier_frozen_mmvae.py -json_file ${json_file} -pt_file ${last_checkpoint} -rid ${random_id} -od ${outdir} -o "LAST_3kEpochs_CC_${name_description}" -cuda True -f ../data/multimodal/240326_nettcr_paired_withswaps.csv -nh 64 -do 0.25 -bn True -n_layers 1 -lr 1e-4 -wd 5e-6 -bs 2048 -ne ${n_epochs} -pepenc 'none' -kf 0 -seed 0 -pepweight True -device cuda:0 >> $logfile 2>&1
    echo "------------------------------------------------------------------------"
    echo ""
done
echo "DONE"
