...
for model_size in "${ms[@]}";do
	for ds in "${datasource[@]}"; do
		...
		if [ "$ds" = "78peps" -o "$ds" = "ExpAll" ]; then
			...
			input_id=(OldDataTop20 OldDataNoPrune OldDataTop15)
		...
		for i in "${!input_df[@]}"; do
			pattern="${ds}_${model_size}"
			# Define the pattern to match to find the model_folder
			match=$(ls /home/projects/vaccine/people/yatwan/tclustr/output/240516_CNNVAE_DiffDatasets/ | grep ${ds} | grep ${model_size})
			filename="$(pwd)/clustering_${ds}_${model_size}_${input_id[i]}.sh"
...
iid=${input_id[i]}
idf=${input_df[i]}
python3 ./240420_VAE_Clustering_intervals.py -np 500 -kf 0 -o \${iid}_CNNVAE_${ds}_${model_size} -od ../output/240516_TripletTweaks_IntervalClustering/ -tbcralign \${tbcralign} -tcrdist \${tcrdist} -f \${idf} -model_folder \${model_folder} -rb True -n_jobs 40 -dn \${iid} -bf ../output/240515_IntervalClustering
EOF
)
