...
OUTNAME="MST_Refined_4VAE_subsampling_${random_id}"
...
while getopts ":f:c:s:l:e:o:i:" opt; do
  case ${opt} in
    f )
      ...
      ;;
    o )
      EXTRA_OUTNAME="${OPTARG}"
      OUTPUTDIRECTORY="${EXTRA_OUTNAME}_${OUTPUTDIRECTORY}"
...
VAEDIR="${HOMEDIR}output/240910_RefinedModels_4vaes/"
# Hardcoded bs
OS_NOTRP_DIR="${VAEDIR}/RefineTest_ONESTAGE_NoTriplet_KFold_0_240910_1842_6omni/"
OS_CSTRP_DIR="${VAEDIR}/RefineTest_TWOSTAGE_NoTriplet_KFold_0_240911_0955_N1jMC/"
TS_NOTRP_DIR="${VAEDIR}/RefineTest_ONESTAGE_CosTriplet_KFold_0_240910_1708_ER8wJ/"
TS_CSTRP_DIR="${VAEDIR}/RefineTest_TWOSTAGE_CosTriplet_KFold_0_240911_0955_jyGpd/"

# loading the last epoch (2500 epoch of refinement ; 25000+2500->27500 total epochs of training)
python3 240903_MST_cuts_clustering_4VAEs.py -pt_file_os_notrp "$(ls $OS_NOTRP_DIR/*2500*.pt)" -json_file_os_notrp "$(ls $OS_NOTRP_DIR/*JSON_kwargs*.json)" -pt_file_ts_notrp "$(ls $TS_NOTRP_DIR/*2500*.pt)" -json_file_ts_notrp "$(ls $TS_NOTRP_DIR/*JSON_kwargs*.json)" -pt_file_os_cstrp "$(ls $OS_CSTRP_DIR/*2500*.pt)" -json_file_os_cstrp "$(ls $OS_CSTRP_DIR/*JSON_kwargs*.json)" -pt_file_ts_cstrp "$(ls $TS_CSTRP_DIR/*2500*.pt)" -json_file_ts_cstrp "$(ls $TS_CSTRP_DIR/*JSON_kwargs*.json)" -f $INPUTFILE -tcrdist $tcrdistfile -tbcralign $tbcrfile -od ${OUTPUTDIRECTORY} -rid 'clstr' -index_col $INDEXCOL -rest_cols "${EXTRACOLS[@]}" -label_col ${LABELCOL} -n_jobs 13 -o $EXTRA_OUTNAME
