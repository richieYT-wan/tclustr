...
margins=(0.05 0.1 0.15 5 10 15)
triplets=("cosine" "cosine" "cosine" "l2" "l2" "l2")
nlatents=(32 64)
wtrps=(0.5 0.75 1)
wklds=(1e-3 1e-4)
  ...
  for nl in "${nlatents[@]}"; do
    for wtrp in "${wtrps[@]}"; do
      for wkld in "${wklds[@]}"; do
        ...
        outname="TripletTuning_${triplets[i]}_${margins[i]}_${nl}_${wtrp}_${wkld}"
        for f in $(seq 0 4); do
...
filename=${filename}
cd \${PYDIR}
python3 ./231102_fulltcr_tripletloss.py -f /home/projects/vaccine/people/yatwan/tclustr/data/filtered/230927_nettcr_positives_only.csv -pad -20 -enc BL50LO -ne 30000 -cuda True -lwseq 3 -lwkld ${wkld} -lwtrp ${wtrp} -dist_type ${triplets[i]} -margin ${margins[i]} -mla1 0 -mla2 0 -mlb1 0 -mlb2 0 -nl ${nl} -nh $((nl * 2)) -bs 512 -lr 1.5e-4 -wd 1e-4 -wu 10 -o ${outname} -rid ${random_string} -kf ${f} -seed ${f}
EOF
)
            ...
            qsub -W group_list=vaccine -A vaccine -m e -l nodes=1:gpus=1:ppn=20,mem=90gb,walltime=${walltime} "/home/projects/vaccine/people/yatwan/tclustr/bashscripts/231201_triplet_tuning/${filename}.sh"
