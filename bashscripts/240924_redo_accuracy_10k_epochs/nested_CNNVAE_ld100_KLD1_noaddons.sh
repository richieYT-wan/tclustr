...
outname=ld100_KLD1_no_addons

# Set the flat phase to 8k epochs and warm-up to 0 and decrease to 0
python3 ./240515_cnnvae_tripletloss.py -f ${HOMEDIR}data/filtered/240618_nettcr_exp_nested_posonly_train_p0234.csv -od ${outdir} -pad -20 -enc BL50LO -ne 10000 -cuda True -lwseq 1 -lwkld 1 -lwtrp 0 -mla1 7 -mla2 8 -mla3 22 -mlb1 6 -mlb2 7 -mlb3 23 -mlpep 0 -nl 100 -nh 100 -bs 512 -lr 1e-4 -wd 1e-4 -wu 0 -fp 10000 -kld_dec 0 -kldts 0.075 -o ${outname} -kf 0 -seed 0 -addpe False -posweight False -bn True -pepweight False -rid ${random_id} -tf ${HOMEDIR}data/filtered/240618_nettcr_exp_nested_posonly_test_p1.csv
