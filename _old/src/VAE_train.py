...
def train_model(model, criterion, optimizer,epoch, dataset, 
    ...
    for b in tqdm(BatchSampler(RandomSampler(dataset),
        ...
        sequences = dataset[b]
        #print(next(model.children())[0].weight.data.device)
        #Sending everything to device
        onehot = onehot_batch(dataset[b], max_len = max_len, weighted = weighted, pad=pad,positional = positional, atchley=atchley).to(device, dtype=torch.float32)
    ...
    train_loss /= math.ceil((len(dataset)/batchsize))
    return train_loss


def eval_model(model, criterion, dataset, batchsize, max_len,
               weighted, pad, positional, atchley, device):
    model.eval()
    criterion.eval()
    val_loss= 0
    with torch.no_grad():
        for b in tqdm(BatchSampler(RandomSampler(dataset),batch_size = batchsize, 
                                   drop_last=False),
                      desc = 'Valid batch',
                      leave =False, position = 4):
                      #d#esc='Val batches', position = 2, leave=False): 
            #Standard train loops
            sequences = dataset[b]
            #Sending everything to device
            onehot = onehot_batch(dataset[b], max_len = max_len, weighted = weighted, pad=pad,
    ...
    val_loss /= math.ceil(len(dataset)/batchsize) # Divide by the number of batches
    
    return val_loss


def test_model(model, criterion, dataset, batchsize, max_len, weighted, pad, positional, atchley, device):
    ...
    losses = []
    with torch.no_grad():
        for b in tqdm(BatchSampler(RandomSampler(dataset),batch_size = batchsize, drop_last=False),
...
def train_eval(model, criterion, optimizer, train_dataset, valid_dataset, batchsize, 
    ...
    for e in tqdm(range(nb_epochs), leave=False):
        ...
        train_loss = train_model(model, criterion, optimizer, e, train_dataset,
                                 batchsize, max_len, weighted, pad = pad, positional = positional, 
        ...
        if e%1==0:
            _,_ = test_decode(model, np.random.choice(valid_dataset ,3), 1, max_len=max_len, pad = pad, weighted=weighted, positional=positional, atchley = atchley, tqdm_write = True)    
...
def tune_vae(train_dataset,
    ...
    criterion = VAELoss(how, alpha, beta, cyclic, hyperbolic, gamma)
    ...
    VAE_redo = VAE_tune(seq_len = 23, aa_dim = aa_dim, latent_dim = latent_dim, act=act)
    VAE_redo.to(device)
    ...
    VAE_redo.load_state_dict(state_dict)
    ...
    test_decode(VAE_redo, test_dataset, n=5, max_len = 23, weighted = weighted, pad = pad,
