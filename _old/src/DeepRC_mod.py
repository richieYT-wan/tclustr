...
class SequenceEmbeddingCNN(nn.Module):
    def __init__(self, n_input_features: int, kernel_size: int = 4, n_kernels: int = 32, n_layers: int = 1, dropout=False):
        ...
        for i in range(self.n_layers):
            ...
            #HERE ADDED NEW
            network.append(nn.Dropout(p=0.2))
...
class DeepRC_mod(nn.Module): 
    ...
    def __init__(self, #20 for the initial onehot encode dim
        ...
        self.embedding = embedding_net.to(dtype=torch.float32) # Here replace Embedding by VAE/AE
    ...
    def forward(self, x, n_per_bag):
        seq_embed = self.embedding(x) # HERE = Values vector
        ...
        emb_after_attention_per_bag = torch.stack(emb_after_attention_per_bag, dim = 0)
        pred_a = self.out_A(x)
        pred_b = self.out_B(x)
