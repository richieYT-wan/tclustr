"""
Towards the end of the file are the old preprocessing functions that are specific to DeepCAT
I kept them here for now, they may be useful later.
Just look for "SEPARATION" (CTRL+F/cmd+F)
"""
...
import pandas as pd 
import warnings
from src.pickling import load_all, load_pkl, load_dict
...
#_, _, _, _, _, hla_a, hla_b = load_all(PATH) #See src.pickling.py

hla_a, hla_b, minmax_atchley = load_dict(PATH)
# Adding a blankspace as a character makes the index start at 1. 
# i.e. '' maps to 0, A maps to 1, C maps to 2, etc
# This way, I can recover a padded sequence
# which is made up of zeros and decode the padded sequence.
# 
AA_KEYS = ['X', 'A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L',
           'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y']

CHAR_TO_INT = dict((c,i) for i,c in enumerate(AA_KEYS))
INT_TO_CHAR = dict((i,c) for i,c in enumerate(AA_KEYS))

POSITION_KEYS = ['start', 'center','end','pad']

def atchley_encoding(seq, max_len=23):
    """Reads a string (seq), Encodes the AA indices to a given sequence with atchley+aaidxPCA factors merged"""
    n_aa = len(seq)
    len_ = max_len-n_aa
    seq = seq + len_* 'X'
    
    temp = np.zeros([len(seq), 5], dtype=np.float32)
...
def atchley_batch(sequences, max_len=23):
    #unsqueeze for channel dimensions
    return torch.stack([atchley_encoding(s, max_len) for s in sequences])#.unsqueeze(1)
    
def quantize(x, threshold):
    """from score to class"""
    thr = torch.nn.Threshold(threshold, 0)#set <threshold to 0
    binary = thr(x)
    binary[binary>0]=1
    return binary.detach().cpu().long()
    
    
def onehot_aa(seq,  max_len =23, weighted = None, pad = 'before', positional = False, atchley = False):
    """
    Encodes a single sequence to one-hot. 
    Encoding can be weighted for first/last 4 amino acids to give it more/less weight+-
    Pad : Pads the sequence to `pad`. Specify None to not pad 
    Position : Add positional encoding 
    """
    original_length = len(seq)
    n = 3 #3 dim if pad is none
    if pad == 'before':
        if max_len is not None:
            len_ = max_len - len(seq)
            seq = seq + len_ *'X' #Padding with X before encoding
            n = 4
    ...
    #onehot_encoded = 
    encoded = torch.tensor(onehot_encoded, dtype=torch.float32)
    if weighted is not None:
        encoded[0:4, :] *= weighted
        encoded[original_length-4:original_length, :] *= weighted
        
    if pad == 'after': # this is the "old way" of doing it, onehot-encode then pad with zeros
        if max_len is not None:
            zeros = torch.zeros((max_len-len(seq)), 21)
            encoded = torch.cat((encoded, zeros))
            n = 4 #dim = 4 if padding is true
    
    # Positional encoding 
    if positional == True:
        pos_encode = torch.zeros((encoded.shape[0], n))
        pos_encode[0:4,0] = 1 # First 4 amino acids (N-Terminal)
        pos_encode[4:original_length-4, 1] = 1 # Centre
        pos_encode[original_length-4:original_length, 2] = 1 # Last 4 amino acids (C-Terminal)
        if max_len is not None:
            pos_encode[original_length:, 3] = 1 #padding
        encoded = torch.cat((encoded, pos_encode), dim=1)
    
    if atchley == True :
        atch_encode = atchley_encoding(seq)
        if max_len is not None:
            zeros = torch.zeros((max_len-len(seq)),5)
            atch_encode = torch.cat((atch_encode,zeros))
        encoded = torch.cat((encoded, atch_encode), dim = 1)
        
    return encoded
    
def onehot_batch(sequences, max_len = 23, weighted= 1.6, pad = 'before', positional = False, atchley = False):
...
def batch_aa_vj(dataset, max_len, weighted, pad, positional, atchley, device):
    ...
    dataset should be an array of dim (N_batch, 3), with 
    dataset[:,0] == sequences, 
    dataset[:,1] == V, dataset[:,2] == J
    """
    onehot = onehot_batch(dataset[:,0], max_len = max_len, weighted = weighted, pad=pad,
                              positional = positional, atchley=atchley).to(device, dtype=torch.float32)
    v_family = F.one_hot(torch.tensor(dataset[:,1].astype(np.int64)), 
                                      num_classes = 30).to(device, dtype=torch.float32)
    j_family = F.one_hot(torch.tensor(dataset[:,2].astype(np.int64)), 
                                          num_classes = 2).to(device, dtype=torch.float32)
    x = (onehot, v_family, j_family)
    return x 
    

def onehot_decode(onehot):
    """decode from a padded one-hot to AA sequence (str)"""
    return ''.join([INT_TO_CHAR[key.item()] for key in torch.argmax(onehot,dim=1)])

def decode_batch(onehot_seqs):
    lst = []
    for seq in onehot_seqs:
        lst.append(onehot_decode(seq))
    return np.array(lst, dtype=object)


def reconstruct_seq_vj(x_true, x_hat, max_len, weighted, positional, atchley):
    """Reconstructs sequences and VJ gene from the vectors and returns the vectors as numpy arrays & reconstructed sequences"""
    x_aa, x_v, x_j = x_hat
    ...
    x_aa = x_aa.detach().cpu()
    onehot = onehot.detach().cpu()
    ...
    x_true = (onehot.numpy(), true_v, true_j)
    x_hat = (decoded.numpy(), x_v, x_j)
    
    return x_true, x_hat, seq_original, seq_reconstructed
...
def get_patient_topk_sequences(df, top_k=10000):
    """for the Emerson dataframe, sorts by sequence clonotype frequency, then take Min(n_sequences_in_patient, top_k) sequences and return the subsampled df"""
    top = df.sort_values('frequency', ascending=False)\
            .groupby('filename').head(top_k)\
            .sort_values('filename', ascending=True)
    return top

def get_ab_label(a1, a2, b1, b2):
    """
    Used in Dataloader/dataset
    Takes the known HLA a1, a2, b1, b2 alleles (str)
    Computes the A and B labels as 1hot encoded vector.
    
    For example, if a patient has allele 'A03', 'A11', for HLA-A,
...
def get_all_label(a1_array, a2_array, b1_array, b2_array):
    ...
    #Else, if it's an array, list, tuple, whatever iterable, do this instead
        ...
        a = torch.stack([x[0] for x in labs]).float()
        b = torch.stack([x[1] for x in labs]).float()
    return a, b
