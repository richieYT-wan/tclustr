...
class EmersonDataset(Dataset):
    ...
    def __init__(self, path, max_len = 23, scale_by_freq = False):#, tokenizer):
        emerson = pd.read_csv(path, sep='\t', header=0, usecols = ['amino_acid','rel_freq',
                                                                   'hla_a1','hla_a2','hla_b1','hla_b2',
                                                                   'true_cmv','filename','len']).query('true_cmv != "Unknown"')
        ...
        self.label_cmv = emerson.true_cmv.astype(int).values
    ...
    def __getitem__(self,index):
        ...
        label_a, label_b = get_all_label(self.hla_a1[index],
                                       ...
                                       self.hla_b1[index],
                                       self.hla_b2[index])
        label_cmv = self.label_cmv[index]
        #Different handling for a single element
        if type(seqs) == str: 
            label_a = label_a.unsqueeze(0)
            label_b = label_b.unsqueeze(0)
            encoded = pad_seq(onehot_aa(seqs), max_len = self.max_len).unsqueeze(0)
            if self.scale ==True:
                scale = self.rel_freq[index]
                encoded = encoded * scale
        #Multi element (batch)
        else: 
            encoded = onehot_batch(seqs, max_len = self.max_len)
            if self.scale ==True:
                scale = self.rel_freq[index]
                encoded = encoded * scale[:,None,None]
        
        return encoded.to(dtype=torch.float32), label_a.to(dtype=torch.float32), label_b.to(dtype=torch.float32), label_cmv.to(dtype=torch.float32)
    ...
    def get_patient(self, patient_filename, how = 'HLA'):
        ...
        if how == 'HLA':
            encoded, label_a, label_b, _ = self.__getitem__(index)
            return encoded, label_a[label_idx], label_b [label_idx], num_per_patient
        
        elif how == 'CMV':
            encoded, _, _, label_cmv = self.__getitem__(index)
...
def load_naive_dataset(path='../training_data_new/db/db_TRB.csv', subset = 1 , ratio_test=0.2, ratio_valid=0.25, max_len = 23):
    """
    Crude function to :
    - read the naive dataset,
    - subsample
    - filter sequences that are both in train test valid etc.
    """
    if subset == 1 :
        df = pd.read_csv(path, header=0)#, nrows=1e5)
    else : 
        df = pd.read_csv(path, header=0).sample(frac=subset)
    df = df.dropna(subset =['cdr3_TRB'])[['cdr3_TRB','TRBV','TRBJ']]
    df = df.query('cdr3_TRB.str.len()>10 and cdr3_TRB.str.len() <= @max_len')
    df = df.query('cdr3_TRB.str.startswith("C")')
    train = df.sample(frac=(1-ratio_test))
    #getting those that are not in the fraction sampled in train
    test = df.loc[df.index.difference(train.index)].copy() 
    #Filter duplicated sequences that are still in test and train set
    train = train.drop(index=train.query('cdr3_TRB in @test.cdr3_TRB.values').index)
    #Sampling valid set
    valid = train.sample(frac=ratio_valid)
    train = train.loc[train.index.difference(valid.index)] #update train
    #dropping validation 
    train = train.drop(index=train.query('cdr3_TRB in @valid.cdr3_TRB.values').index)
    return train.cdr3_TRB.values, valid.cdr3_TRB.values, test.cdr3_TRB.values
